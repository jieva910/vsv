...
$ticktNum = "INC0132706"
 ...
 $CSVfile2 = Import-Csv -LiteralPath C:\Temp\itms.csv -Delimiter ","

 $cmp = New-Object -COMObject 'SAPbobsCOM.Company'


function fn_FrozenItems {
    param ($itemcode)
     $oitemcode=$cmp.getbusinessobject(4)
    if ( $oitemcode.GetByKey($itemcode) ) {
        $oitemcode.valid = 0 
        $oitemcode.Frozen = 1
        #$oitemcode.FrozenFrom = get-date -Format "yyyy-MM-dd"
        #$oitemcode.FrozenTo = "2999-12-31"
        $oitemcode.FrozenRemarks = $ticktNum
       
        $outputcode = $oitemcode.Update()
        $outlog =$cmp.CompanyDB + ' '+ $itemcode + ' deactivated  with error code:' +   $outputcode + ' and error description is:' + $cmp.GetLastErrorDescription()
 ...
 $SAP_SiteConnS.Keys -match $site | Sort-Object | ForEach-Object { 
        ...
        if(-not $cmp.Connected) {Write-Host $cmp.GetLastErrorDescription() 
         ...
         Write-Host "Start processing ......"
          ...
          #Deactivate itemcodes in CSV file
         ForEach ($row in $CSVfile2){           
                    try { $outlog2=fn_FrozenItems  $row.itemcode }
