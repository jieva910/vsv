...
#>
 ...
 $SQL="SELECT (SELECT manager FROM OADM) sitecode ,T.USERID,T.USER_CODE,T.U_NAME,t.lastLogin FROM OUSR t WHERE t.SUPERUSER = 'y',(select Version from cinf) SapVersion AND t.GROUPS <>99 AND t.Locked ='N'"
...
{ 
  ...
  $EndTime=Get-Date
 $hash.Keys |Sort-Object | ForEach-Object{  $hash[$_]  }  | export-csv C:\TEMP\SUPER_USER_INSAP.csv -NoTypeInformation
