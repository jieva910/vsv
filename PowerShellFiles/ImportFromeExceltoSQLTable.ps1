...
function fn_ConvertExl2CSV($sourceFile)
{ 
  
$global:finalCsvFile=$sourceFile.Replace('.xlsx','.CSV')
$tempfinalCsvFile ="C:\Temp\temp.csv"
...
$workbook.SaveAs($tempfinalCsvFile,6)  # 6 corresponds to the CSV format
...
$content = Get-Content -Path $tempfinalCsvFile -Raw
[System.IO.File]::WriteAllText($finalCsvFile,$content,[System.Text.Encoding]::UTF8)
Remove-Item -Path $tempfinalCsvFile
...
function ImportFromCSVtoSQL($sourceFile)
{
 ...
 try {
	  ...
	  fn_ConvertExl2CSV $sourceFile
       $csv = import-csv  $finalCsvFile -Delimiter ","  #'C:\dell\CS VAT list2.csv'
# group by sdnum
