...
function fn_UpdateItem {
    param ($cmp ,$itmcode)
     $row_data=$itmcode -split ","
     $oItm=$cmp.getbusinessobject(4)
    if ( $oItm.GetByKey($row_data[0].Trim()) ) {
        $oItm.ItemName =$row_data[1]
        $oItm.GTSItemSpec = $row_data[2]
		 $oItm.User_Text =$row_data[3]
...
#----OPEN UDF CSV FILE-------------
 $CSVfile = Get-Content -ReadCount 0 -LiteralPath C:\Temp\csitems.csv

 $SAP_SiteConnS = @{CS =@{ Lic="SZ-SAPLIC92";db="SZ-SAP01";dbtype="8";cmp="SAPB1_CS";sapuser="jieva";pwd="vesint99"}
#TK =@{ Lic="DG-SAPLIC01";db="DG-APAC-SAP01";dbtype="8";cmp="SAPB1_TK";sapuser="jieva";pwd="Ves1234"}
 ...
 $SAP_SiteConnS.Keys | Sort-Object | ForEach-Object { 
    ...
    $hash.keys|Sort-Object | ForEach-Object{ $hash[$_] } | Out-File -Append 'c:\temp\updated_csitems.log'
